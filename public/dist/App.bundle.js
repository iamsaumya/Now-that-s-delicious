...
function typeAhead(search) {
    ...
    var searchInputs = document.querySelector('input[name="search"]');
    ...
    searchInputs.on('input', function (e) {
        ...
        axios.get('/api/search?q=' + this.value).then(function (results) {
            if (results.data.length) {
                searchResults.innerHTML = dompurify.sanitize(searchResultsHTML(results.data));
                return;
            }
            searchResults.innerHTML = dompurify.sanitize('<div class=\'search__results\'>No results found for ' + _this.value + '</div>');
        }).catch(function (err) {
            console.log(err);
        });
    });
